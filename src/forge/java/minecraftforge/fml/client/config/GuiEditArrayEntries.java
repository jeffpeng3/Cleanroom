...
public class GuiEditArrayEntries extends GuiListExtended
{
    ...
    protected void saveListChanges()
    {
        ...
        if (owningGui.slotIndex != -1 && owningGui.parentScreen != null
                ...
                && ((GuiConfig) owningGui.parentScreen).entryList.getListEntry(owningGui.slotIndex) instanceof GuiConfigEntries.ArrayEntry)
        {
            GuiConfigEntries.ArrayEntry entry = (GuiConfigEntries.ArrayEntry) ((GuiConfig) owningGui.parentScreen).entryList.getListEntry(owningGui.slotIndex);
        ...
        public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partial)
        {
            if (this.getValue() != null && this.isValidated)
                owningEntryList.getMC().fontRenderer.drawString(
                        isValidValue ? TextFormatting.GREEN + GuiUtils.VALID : TextFormatting.RED + GuiUtils.INVALID,
                        listWidth / 4 - owningEntryList.getMC().fontRenderer.getStringWidth(GuiUtils.VALID) - 2,
