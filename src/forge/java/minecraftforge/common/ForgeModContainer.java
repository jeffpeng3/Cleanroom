...
package minecraftforge.common;

import minecraftforge.common.config.Config;
import minecraftforge.common.config.ConfigCategory;
import minecraftforge.common.config.ConfigManager;
import minecraftforge.common.config.Configuration;
import minecraftforge.common.config.Property;
import minecraftforge.common.model.animation.CapabilityAnimation;
import minecraftforge.common.network.ForgeNetworkHandler;
import minecraftforge.energy.CapabilityEnergy;
import minecraftforge.fluids.FluidRegistry;
import minecraftforge.fluids.UniversalBucket;
import minecraftforge.fluids.capability.CapabilityFluidHandler;
import minecraftforge.server.command.ForgeCommand;
...
import minecraftforge.classloading.FMLForgePlugin;
import minecraftforge.client.ForgeClientHandler;
import minecraftforge.event.RegistryEvent;
import minecraftforge.event.RegistryEvent.MissingMappings;
import minecraftforge.event.terraingen.DeferredBiomeDecorator;
...
import minecraftforge.oredict.RecipeSorter;
...
import minecraftforge.fml.client.FMLFileResourcePack;
import minecraftforge.fml.client.FMLFolderResourcePack;
import minecraftforge.fml.client.event.ConfigChangedEvent.OnConfigChangedEvent;
import minecraftforge.fml.common.DummyModContainer;
import minecraftforge.fml.common.FMLCommonHandler;
import minecraftforge.fml.common.FMLLog;
import minecraftforge.fml.common.ICrashCallable;
import minecraftforge.fml.common.LoadController;
import minecraftforge.fml.common.Loader;
import minecraftforge.fml.common.Mod.EventBusSubscriber;
import minecraftforge.fml.common.ModMetadata;
import minecraftforge.fml.common.WorldAccessContainer;
...
import minecraftforge.fml.common.event.FMLModIdMappingEvent;
...
import minecraftforge.fml.common.gameevent.PlayerEvent;
...
public class ForgeModContainer extends DummyModContainer implements WorldAccessContainer
{
    ...
    private static void remapGeneralPropertyToClient(String key)
    {
        ...
        if (GENERAL.containsKey(key))
        {
            ...
            GENERAL.remove(key);
            config.getCategory(Configuration.CATEGORY_CLIENT).put(key, property);
        }
    }
    ...
    private static void syncConfig(boolean load)
    {
        ...
        if (config.getCategory(Configuration.CATEGORY_GENERAL).containsKey("defaultSpawnFuzz")) config.getCategory(Configuration.CATEGORY_GENERAL).remove("defaultSpawnFuzz");
        if (config.getCategory(Configuration.CATEGORY_GENERAL).containsKey("spawnHasFuzz")) config.getCategory(Configuration.CATEGORY_GENERAL).remove("spawnHasFuzz");
        if (config.getCategory(Configuration.CATEGORY_GENERAL).containsKey("disableStitchedFileSaving")) config.getCategory(Configuration.CATEGORY_GENERAL).remove("disableStitchedFileSaving");
        if (config.getCategory(Configuration.CATEGORY_CLIENT).containsKey("java8Reminder")) config.getCategory(Configuration.CATEGORY_CLIENT).remove("java8Reminder");
        if (config.getCategory(Configuration.CATEGORY_CLIENT).containsKey("replaceVanillaBucketModel")) config.getCategory(Configuration.CATEGORY_CLIENT).remove("replaceVanillaBucketModel");
        ...
        prop = config.get(Configuration.CATEGORY_GENERAL, "disableVersionCheck", false);
        ...
        prop = config.get(Configuration.CATEGORY_GENERAL, "sortRecipies", true);
        ...
        config.setCategoryPropertyOrder(Configuration.CATEGORY_CLIENT, propOrder);
    ...
    public void modConstruction(FMLConstructionEvent evt)
    {
        ...
        all.add("minecraftforge/common/util/TextTable");
        all.add("minecraftforge/common/util/TextTable$Column");
        all.add("minecraftforge/common/util/TextTable$Row");
        all.add("minecraftforge/common/util/TextTable$Alignment");

        all.removeIf(cls -> !cls.startsWith("net/minecraft/") && !cls.startsWith("minecraftforge/"));
