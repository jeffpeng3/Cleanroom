...
import minecraftforge.common.ForgeHooks;
...
public class GuiIngameForge extends GuiIngame
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        if (pre(RenderGameOverlayEvent.ElementType.ALL)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.CROSSHAIRS)) return;
        bind(Gui.ICONS);
        GlStateManager.enableBlend();
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.POTION_ICONS)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.SUBTITLES)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.BOSSHEALTH)) return;
        bind(Gui.ICONS);
        GlStateManager.tryBlendFuncSeparate(GlStateManager.SourceFactor.SRC_ALPHA, GlStateManager.DestFactor.ONE_MINUS_SRC_ALPHA, GlStateManager.SourceFactor.ONE, GlStateManager.DestFactor.ZERO);
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.HELMET)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.ARMOR)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.PORTAL)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.HOTBAR)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.AIR)) return;
    ...
    {
        bind(ICONS);
        if (pre(RenderGameOverlayEvent.ElementType.HEALTH)) return;
    ...
    {
        if (pre(RenderGameOverlayEvent.ElementType.FOOD)) return;
    ...
    {
        bind(ICONS);
        if (pre(RenderGameOverlayEvent.ElementType.EXPERIENCE)) return;
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        GlStateManager.disableBlend();
    ...
    {
        bind(ICONS);
        if (pre(RenderGameOverlayEvent.ElementType.JUMPBAR)) return;
    ...
    protected void renderPlayerList(int width, int height)
    {
        ...
        if (mc.gameSettings.keyBindPlayerList.isKeyDown() && (!mc.isIntegratedServerRunning() || handler.getPlayerInfoMap().size() > 1 || scoreobjective != null))
        {
            ...
            if (pre(RenderGameOverlayEvent.ElementType.PLAYER_LIST)) return;
    ...
    protected void renderHealthMount(int width, int height)
    {
        ...
        if (pre(RenderGameOverlayEvent.ElementType.HEALTHMOUNT)) return;
